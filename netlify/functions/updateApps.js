...
exports.handler = async (event, context) => {
  try {
    ...
    const newAppsData = JSON.stringify(body, null, 2);
    ...
    await octokit.repos.createOrUpdateFileContents({
      ...
      repo: REPO_NAME,
      path: APPS_FILE,
      message: `Uygulama verileri güncellendi - ${new Date().toLocaleString('tr-TR')}`,
      content: Buffer.from(newAppsData).toString("base64"),
    ...
    return {
      ...
      body: JSON.stringify({ 
        success: true,
        message: "Apps data başarıyla GitHub'a kaydedildi"
      }),
    };
  } catch (error) {
